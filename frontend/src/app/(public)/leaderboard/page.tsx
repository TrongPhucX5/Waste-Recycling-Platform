...
export default function LeaderboardPage() {
  ...
  // Cấu hình style "xịn" cho bục vinh quang (Top 3)
  const getPodiumStyle = (rank: number) => {
    switch (rank) {
      case 1:
        return {
          wrapper: "order-1 md:order-2 z-10 w-full md:w-5/12 transform md:-translate-y-6", // To nhất, nằm giữa, nhô cao
      ...
      case 2:
        return {
          wrapper: "order-2 md:order-1 w-full md:w-4/12 mt-6 md:mt-0 opacity-95 hover:opacity-100", // Bên trái
      ...
      case 3:
        return {
          wrapper: "order-3 w-full md:w-4/12 mt-6 md:mt-0 opacity-95 hover:opacity-100", // Bên phải
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      {/* Header - Chuyển sang gradient hiện đại */}
      ...
      {/* Controls - Thanh công cụ làm nổi (Floating) */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {activeTab === "individual" ? (
              ...
              <>
                  ...
                  {filteredIndividuals.slice(0, 3).map((person) => {
                    ...
                    return (
                      <div key={person.id} className={`${style.wrapper} transition-transform duration-500 ease-out`}>
                        <div className={`relative rounded-3xl p-6 md:p-8 flex flex-col items-center hover:-translate-y-2 transition-all duration-300 ease-out cursor-default ${style.card}`}>
                          ...
                          <div className="flex items-center justify-center gap-1.5 text-sm font-medium text-slate-500 mb-5 bg-slate-50 px-3 py-1.5 rounded-lg w-full">
                ...
                <div className="flex flex-col gap-3">
                  {filteredIndividuals.slice(3).map((person) => (
                    <div 
                      key={person.id} 
                      className="group bg-white rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-center gap-5 border border-slate-200 hover:border-emerald-300 hover:shadow-lg hover:shadow-emerald-900/5 transition-all duration-300 ease-out cursor-default"
                    >
                      <div className="flex-shrink-0 w-14 h-14 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center font-black text-lg text-slate-400 group-hover:bg-emerald-50 group-hover:text-emerald-600 group-hover:border-emerald-200 transition-colors">
                        #{person.rank}
                      </div>
                      ...
                      <div className="flex-1 text-center sm:text-left">
                        <h3 className="font-bold text-lg text-slate-800 group-hover:text-emerald-700 transition-colors">{person.name}</h3>
                      ...
                      <div className="text-center sm:text-right mt-4 sm:mt-0 min-w-[120px]">
                        <div className="text-xl font-black text-emerald-600">{formatNumber(person.points)}</div>
          ...
          /* Area Leaderboard - Cập nhật style xịn */
